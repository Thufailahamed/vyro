  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="space-y-3">
        <div className="relative z-20 bg-paper border border-ink/15 p-2 sm:p-2.5 shadow-sm focus-within:border-ink focus-within:ring-2 focus-within:ring-volt/40 transition-all duration-200 flex items-center gap-3 rounded-xl">
          <div className="size-9 bg-ink text-volt flex items-center justify-center shrink-0 rounded-lg">
            <SearchIcon size={18} />
          </div>
          ...
          <div className="hidden sm:flex items-center gap-1.5 border-l border-ink/10 pl-3 pr-1 text-xs font-mono text-ink-4">
            <kbd className="px-2 py-0.5 bg-mist border border-line text-[10px] rounded-md">Instant Live Filter</kbd>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-ink/10">
        <div className="flex items-center gap-3 text-sm text-ink-3">
          ...
          {fastDispatchOnly && (
            <span className="inline-flex items-center gap-1 text-[11px] font-mono font-semibold px-2 py-0.5 bg-volt/20 text-ink border border-volt/30 rounded-md">
              ⚡ Fast dispatch only
            </span>
          )}
        </div>
      ...
      {isLoading && (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="h-96 bg-paper border border-ink/10 animate-pulse rounded-xl" />
          ))}
        </div>
      )}
      ...
      {!isLoading && viewMode === 'grid' && processedHits.length > 0 && (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {processedHits.map((h) => {
            ...
            const adding = pendingKey === h.bestOffer?.id;

            return (
              <div
                key={h.product.id}
                className="group bg-paper border border-ink/15 hover:border-ink hover:shadow-lg transition-all duration-200 flex flex-col justify-between overflow-hidden relative rounded-xl"
              >
                {/* Image Section */}
                <Link
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  <div className="pt-3 border-t border-ink/10 flex items-center gap-2">
                    <Link
                      to={`/products/${h.product.id}`}
                      className="vyro-btn vyro-btn-secondary flex-1 text-xs uppercase tracking-wider font-bold py-2.5 text-center"
                    >
                    ...
                    {h.bestOffer && (
                      <Button
                        type="button"
                        size="sm"
                        className="text-xs uppercase tracking-wider font-bold py-2.5 h-auto"
                        loading={adding}
                        onClick={() =>
                          addToCart({
    ...
    <div className="flex flex-wrap items-center gap-1.5 pt-1" aria-label="Active NL filters">
      ...
      {chips.map((c, i) => (
        <button
          ...
          className="inline-flex items-center gap-1 h-7 pl-2.5 pr-1.5 text-[11px] font-mono border border-ink/20 bg-bone hover:bg-ink hover:text-volt text-ink-3 transition-colors rounded-md"
          title="Remove filter"
        >
          <span>{c.label}</span>
          <XIcon size={11} />
