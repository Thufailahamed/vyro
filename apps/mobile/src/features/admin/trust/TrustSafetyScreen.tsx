      ...
      {tab === 'reports' ? (
        <>
          <ChipRow
            ...
            options={['open', 'investigating', 'resolved', 'dismissed', 'all'].map((s) => ({
              value: s,
              label: humanize(s),
            }))}
          />
          {reports.isLoading ? (
            ...
            <EmptyState
              icon={FileWarning}
              title="Queue clear"
              message="No abuse reports in this state."
            />
          ) : (
            <View style={{ gap: 10 }}>
              {(reports.data?.items ?? []).map((r, i) => (
                <Appear key={r.id} i={i % 10}>
                  <RecordCard
                    ...
                    title={humanize(r.reason)}
                    subtitle={humanize(r.targetType)}
                    meta={`${formatDateTime(r.createdAt)}${r.assignedTo ? ' · claimed' : ''}`}
                    status={<StatusBadge status={r.status} size="sm" />}
                    chips={<Pill label={`Target ${r.targetId.slice(0, 10)}`} />}
                    actions={
                        ...
                        <Can perm="abuse_report:resolve">
                          ...
                          {r.status !== 'resolved' ? (
                            <Button
                              ...
                              size="sm"
                              onPress={() => {
                                setResolveTarget({ report: r, resolution: 'resolved' });
                                setNote('');
                              }}
                            />
                          ) : null}
                          {r.status !== 'dismissed' && r.status !== 'resolved' ? (
                            <Button
                              ...
                              variant="ghost"
                              onPress={() => {
                                setResolveTarget({ report: r, resolution: 'dismissed' });
                                setNote('');
                              }}
                            />
                          ) : null}
        ...
        <>
          {kyc.isLoading ? (
            ...
            <EmptyState
              ...
              message="No pending identity reviews."
            />
          ) : (
            <View style={{ gap: 10 }}>
                      ...
                      <Can perm="kyc:review">
                        <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
                          <Button
                            ...
                            size="sm"
                            onPress={() => {
                              setKycTarget({ row: k, decision: 'approved' });
                              setNote('');
                            }}
                          />
                          <Button
                            ...
                            variant="danger"
                            onPress={() => {
                              setKycTarget({ row: k, decision: 'rejected' });
                              setNote('');
                            }}
                          />
                          <Button
                            ...
                            variant="paper"
                            onPress={() => {
                              setKycTarget({ row: k, decision: 'needs_more_info' });
                              setNote('');
                            }}
                          />
                        </View>
      ...
      >
        <Field label="Resolution notes" hint="Stored with the report">
          <Input
            ...
            multiline
            placeholder="Evidence reviewed, action taken…"
          />
        </Field>
      </Sheet>
