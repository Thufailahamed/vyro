...
describe('intent allowlist', () => {
  it('exports 31 intents in INTENT_NAMES (24 base+intel+planner + 6 rfq + finance_status)', () => {
    expect(INTENT_NAMES.length).toBe(31);
    for (const i of ['create_rfq', 'compare_quotes', 'rfq_invite_suppliers', 'rfq_negotiate', 'rfq_recommend_quote', 'rfq_status', 'finance_status'] as const) {
  ...
  it('viewer can run read-only intents', () => {
    const allowed: IntentName[] = [
      ...
      'spend_forecast', 'category_intel', 'insights_feed', 'finance_status',
    ];
  ...
  it('money-status utterances route to finance_status (read-only)', () => {
    const dict = { products: ['Samba Rice'], suppliers: ['Mill A'] };
    expect(heuristicClassify('show me pending payments', dict).intent).toBe('finance_status');
    expect(heuristicClassify('which invoices are unpaid', dict).intent).toBe('finance_status');
    expect(heuristicClassify('how much do we owe suppliers', dict).intent).toBe('finance_status');
  });
